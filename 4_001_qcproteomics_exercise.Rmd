...
output:
  html_document:
    css: d:/r_projects/hp_r_course/exercises/exercises_style.css
---
...
#nbt1328_S3 <- readr::read_delim(file = file.path(root, "data", "nbt1328-S3.txt"),
#                                delim = "\t", skip = 2)
nbt1328_S2 <- readr::read_delim(file = file.path(root, "data", "nbt1328-S2.txt"),
                                delim = "\t", skip = 4)
...
```{r}
names(nbt1328_S2) <- stringr::str_replace_all(names(nbt1328_S2), pattern = " ", 
                                                                 replacement = "_") %>%
  tolower()
...
```{r}
## load data
labelling <- read_delim(file.path(root, "data", "labellings_qcproteomics.txt"),
                         delim = " ") 
...
head(labelling)
names(labelling) <- c("experiment_id", "cell_line", "method", "chemical", "profile")
head(labelling)
labelling
...
For the heatmap we use the package `pheatmap`. There are a lot of other packages in R you can use.
```{r, fig.height= 16}
?pheatmap
...
png("let_s_go_crazy_heatmap.png",
